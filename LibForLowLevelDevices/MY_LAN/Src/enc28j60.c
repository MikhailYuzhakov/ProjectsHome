...
{
 SS_SELECT();
 SPI_SendByte(op|(addres&ADDR_MASK)); //маска адреса обнуляет 3 старших бита для опкода
 SPI_SendByte(data);
 SS_DESELECT();
}
//--------------------------------------------------
...
void enc28j60_ini(void)
{
	...
	enc28j60_writeRegByte(MACON1,MACON1_MARXEN|MACON1_TXPAUS|MACON1_RXPAUS); //включаем биты. MARXEN - разрешает приниматься пакеты, TXPAUS и RXPAUS - включают аппаратное управление потоком
	enc28j60_writeRegByte(MACON2,0x00);
	enc28j60_writeOp(ENC28J60_BIT_FIELD_SET,MACON3,MACON3_PADCFG0|MACON3_TXCRCEN|MACON3_FRMLNEN); //PADCFG[2:0] биты отвечают за паддинг или выравнивание длины пакета до 60 байт (и контрольную сумму), TXCRCEN - автоматически добавляет к пакеты контрольную сумму, FRMLEN - автоматически проверяет длину пакетов.
	...
	enc28j60_writeRegByte(MAADR5,macaddr[0]);//устанавливаем MAC-адрес нашего устройства (микросхемы)
	...
	enc28j60_writePhy(PHLCON,PHLCON_LACFG2|PHLCON_LBCFG2|PHLCON_LBCFG1|PHLCON_LBCFG0|PHLCON_LFRQ0|PHLCON_STRCH); //настраиваем светодиоды
	enc28j60_SetBank(ECON1);
	enc28j60_writeOp(ENC28J60_BIT_FIELD_SET,EIE,EIE_INTIE|EIE_PKTIE); //установим биты, разрешающие глобальные прерывания и приём пакетов по ожиданию прерывания
...
{
	uint16_t len = 0;
	if(enc28j60_readRegByte(EPKTCNT) > 0) //в регистре EPKTCNT хранится кол-во принятых пакетов на данный момент
	{
		enc28j60_writeReg(ERDPT,gNextPacketPtr); //в ERDPT записываем указатель на следующий пакет
		//считаем заголовок
		struct{
		  uint16_t nextPacket;
		  uint16_t byteCount;
...
{
	while(enc28j60_readOp(ENC28J60_READ_CTRL_REG,ECON1)&ECON1_TXRTS) //ждем пока наша микросхема будет готова к передаче
	{
